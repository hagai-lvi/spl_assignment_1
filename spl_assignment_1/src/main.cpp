	...
	*/
	...
	for (int i=0; i<products.size(); i++) {
		...
		for (int j=0 ; j<products[i]->ingredients.size() ; j++) {
			cout << products[i]->ingredients[j]->name << ", " << products[i]->ingredients[j]->supplier << " | ";
		}
	...
	vector<Ingredient*> shoppingList = makeShoppingList(products);
	cout <<"******************************** PRINTING THE SHOPPING LIST" << endl;
	for (int i=0 ; i<shoppingList.size() ; i++) {
		cout << "supplier: "<< shoppingList[i]->supplier << ", name: " << shoppingList[i]->name << endl;
	}
	cout <<"******************************** Finished printing the shopping list" << endl;
	vector<string> shoppingListString = makeTheShoppingList(shoppingList);
	cout << shoppingListString.size();
	
	writefile_H(shoppingListString ,"ShoppingList.out");
